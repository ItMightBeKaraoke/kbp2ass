        ...
        if (currentEnd < 0) currentEnd = 0;
			...
			if (line.replace(/\s*/g, '').length == 0) {
				if (syllables.length > 0) {
					...
					let sentence = this.makeSentence(sentenceID, syllables, currentStart, currentEnd, currentStyle.Name, currentPos + currentOffset, horizontalPos, currentAlignment == currentStyle.Alignment ? 0 : currentAlignment);
					currentStart = null;
					currentEnd = null;
